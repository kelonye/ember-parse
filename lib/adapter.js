...
//
var EmberParseAdapter = module.exports = {};
...
EmberParseAdapter.Serializer = DS.RESTSerializer.extend({
  ...
  extractArray: function(store, primaryType, payload){
    ...
    //       item[key] = val.map(function(record){
  ...
  extractMeta: function(store, type, payload) {
    if (payload && payload.count) {
      ...
      store.setMetadataFor(type, {count: 0});
    }
  },
  ...
  serializeHasMany: function(record, json, relationship){
    ...
    if(hasMany && hasMany.get('length') > 0){
      ...
      hasMany.forEach(function(child){
        json[key].push({
          "__type": "Pointer",
          "className": serializer.parseClassName(child.__proto__.constructor.typeKey),
...
EmberParseAdapter.Transforms.Date = DS.Transform.extend({
  ...
  deserialize: function(serialized) {
    ...
    return new Date(serialized.iso || serialized);
  },
